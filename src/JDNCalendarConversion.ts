...
export module JDNConvertibleConversion {
    ...
    export class JDNCalendarConversion {
         ...
         * @returns {number}
         */
        public static gregorianToJDN(calendarDate: JDNConvertibleCalendar.CalendarDate): number {
         ...
         */
        public static JDNToGregorian(jdn: number): JDNConvertibleCalendar.CalendarDate {
            ...
            const yearday = wjd - JDNCalendarConversion.gregorianToJDN(new JDNConvertibleCalendar.CalendarDate(year, 1, 1));
            const leapadj = ((wjd < JDNCalendarConversion.gregorianToJDN(new JDNConvertibleCalendar.CalendarDate(year, 3, 1))) ? 0 : (JDNCalendarConversion.leapGregorian(year) ? 1 : 2));
            const month = Math.floor((((yearday + leapadj) * 12) + 373) / 367);

            const day = (wjd - JDNCalendarConversion.gregorianToJDN(new JDNConvertibleCalendar.CalendarDate(year, month, 1))) + 1;
         ...
         * @returns {number}
         */
        public static julianToJDN(calendarDate: JDNConvertibleCalendar.CalendarDate) {
         ...
         * @returns {JDNConvertibleCalendar.CalendarDate}
         */
        public static JDNToJulian(jdn: number): JDNConvertibleCalendar.CalendarDate {
